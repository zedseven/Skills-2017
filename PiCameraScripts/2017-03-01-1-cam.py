...
with picamera.PiCamera() as camera:
    ...
    fullMoveTime = 1.9849081365#2.20#2.30 #time taken to move forward fullMoveDistance in seconds
    fullMoveDistance = 27.50#30.48 #distance in cm
    fullTurnTime = (2.9 * 2)#4 #time taken to perform a full 360° turn in seconds
    forwardSensitivity = 7 #how close to the center a football must be before going to pick it up
    fullMoveSize = 115 #tested with 150
    pickupSize = 2300#1500 #2419 #tested with 2500
    pickupSensitivity = 500 #how close to the ideal football screen size a football should be before pickup
    ...
    modulo = 8
    drawToScreenPs = 8
    brownBuffer = 0.02#0.05#0.02
    brown = (113, 97, 92)#(79, 62, 29)#(113, 100, 53)#(83, 64, 27)#(59, 44, 18)#(153, 142, 110)#(128, 90, 62)#(143, 111, 107)#(153, 111, 107)#(79, 80, 79)#(81, 55, 34)#(102, 26, 0)#(115, 98, 103)#(84, 61, 42)#(139, 69, 19)
    ...
    while 1 == 1:
        ...
        with picamera.array.PiRGBArray(camera) as stream:
            ...
            if clusterCount > 0:
                ...
                moveTime = moveAmnt * fullMoveTime #(fullMoveTime * (15.1 / 14.089))
                turned = False
                moved = False
                if turnAmnt >= forwardSensitivity: #turn right
                    ...
                    if targetCluster[1] < (pickupSize - pickupSensitivity):
                        ...
                        lastGPIO = pickupGPIO
                time.sleep((turnTime if turned == True else moveTime))
                ...
                if greenPixelCount > greenPixelMin:
                    ...
                    print('Nothing was found, so we\'re going to go in the same direction as we did before.')
                    newGPIO = oppositeGPIO(lastGPIO)
                    gpio.output(newGPIO, 1)
                    time.sleep(notFoundMoveTime)
                    gpio.output(newGPIO, 0)
